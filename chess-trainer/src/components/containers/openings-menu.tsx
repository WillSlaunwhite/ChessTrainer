...
const OpeningsMenu: React.FC<{ setFen: (fen: string) => void}> = ({setFen}) => {
	const openings = {
	...
	return (
		<div className="menu-container w-full flex items-center justify-center flex-col gap-1 mt-1">
			...
			<Card className="w-10/12">
				<List className="mt-1">
					<ListItem className="ripple-bg-blue-700 ripple" onClick={() => setFen(openings["Italian"])}>Italian Game</ListItem>
					<ListItem className="ripple-bg-blue-700 ripple" onClick={() => setFen(openings["Ruy"])}>Ruy Lopez</ListItem>
					<ListItem className="ripple-bg-blue-700 ripple" onClick={() => setFen(openings["Scotch"])}>Scotch Game</ListItem>
				</List>
			</Card>
			<Card className="w-10/12 bg-black mt-1">
				<List>
					<ListItem className="text-light-400 ripple-bg-blue-700 ripple"onClick={() => setFen(openings["Sicilian"])}>Sicilian Defense</ListItem>
					<ListItem className="text-light-400 ripple-bg-blue-700 ripple"onClick={() => setFen(openings["French"])}>French Defense</ListItem>
					<ListItem className="text-light-400 ripple-bg-blue-700 ripple"onClick={() => setFen(openings["Caro-Kann"])}>Caro-Kann</ListItem>
				</List>
			</Card>
		</div>
	);
